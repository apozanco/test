...
def pddl_to_sas(task):
    ...
    if options.domain_from_c:
        import json
        print('Reading C.json')
        with open('C.json') as json_file:
            cost_function = json.load(json_file)
        for action in actions:
            action.cost = int(cost_function[action.name])

    # ALBERTO
    print(f'Writing Grounded actions in {os.getcwd()}')
    grounded_actions = []
    grounded_actions_dict = {}
    for action in actions:
        grounded_actions.append(action.name)
        grounded_actions_dict[action.name] = action.cost
    outfile = open('grounded_actions.txt','w+')
    outfile.write(','.join(grounded_actions))
    outfile.close()
    import json
    with open('grounded_actions_with_cost.json', 'w') as fp:
        json.dump(grounded_actions_dict, fp)
